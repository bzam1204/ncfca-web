...
import { AffiliationDetailsDialog } from '@/app/(admin)/admin/dashboard/affiliations/_components/affiliations-details-dialog';
...
export function AffiliationsTable({ initialData }: { initialData: AffiliationDto[] }) {
  ...
  return (
    <>
          ...
          <TableBody>
            {affiliations.map((affiliation) => (
              <TableRow key={affiliation.id} onClick={() => setSelectedAffiliation(affiliation)} className="cursor-pointer hover:bg-muted/50">
                ...
                <TableCell>
                  <Badge variant={getFamilyStatusVariant(affiliation.status)}>{familyStatusTranslation[affiliation.status]}</Badge>
                </TableCell>
      ...
      <AffiliationDetailsDialog isOpen={!!selectedAffiliation} onClose={() => setSelectedAffiliation(null)} affiliation={selectedAffiliation} />
    </>
  );
}
