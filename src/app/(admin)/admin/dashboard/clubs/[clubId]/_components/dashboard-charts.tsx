...
export function DashboardCharts({clubId}: DashboardChartsProps) {
  ...
  const genderConfig = {
    ...
    ...Object.fromEntries(chartsData.demographics.gender.map(g => [g.name, {
      ...
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <DistributionChart title="Distribuição por Gênero" description="Composição atual do clube"
                             data={chartsData.demographics.gender} config={genderConfig} />
          <DistributionChart title="Distribuição por Faixa Etária" description="Composição etária do clube"
                             data={chartsData.demographics.age} config={ageConfig} />
        </div>
