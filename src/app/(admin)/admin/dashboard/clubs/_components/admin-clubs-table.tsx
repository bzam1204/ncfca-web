    ...
    <Card>
      ...
      <CardContent className="space-y-6">
            ...
            <TableBody>
              {isLoading || isFetching ? (
                ...
                paginatedData.data.map((club) => (
                  <TableRow key={club.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleRowClick(club.id)}>
                    <TableCell className="font-medium hover:underline">{club.name}</TableCell>
                    <TableCell>
        ...
        {paginatedData?.meta && paginatedData.meta.totalPages > 1 && (
          <PaginationControls currentPage={filters.page!} totalPages={paginatedData.meta.totalPages} onPageChange={handlePageChange} />
        )}
      </CardContent>
    </Card>
  );
