      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Digite uma descrição para o treinamento" className="min-h-[100px] overflow-y-auto max-h-[300px]" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex gap-4 pt-4">
          <Button type="submit" disabled={isLoading} className="flex-1">
            {isLoading ? (isEditing ? 'Atualizando...' : 'Criando...') : isEditing ? 'Atualizar Treinamento' : 'Criar Treinamento'}
          </Button>
        </div>
      </form>
    </Form>
...
const trainingSchema = z.object({
  ...
  youtubeUrl: z
    ...
    .refine((url) => url.includes('youtube.com') || url.includes('youtu.be'), 'URL deve ser do YouTube'),
});
