    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between">
          {value ? states.find((state) => state.value.toLowerCase() === value.toLowerCase())?.label : 'Selecione o estado...'}
          ...
          <CommandList>
            ...
            <CommandGroup>
              {states.map((state) => (
                ...
                >
                  {state.label}
                  <Check className={cn('ml-auto', value?.toLowerCase() === state.value.toLowerCase() ? 'opacity-100' : 'opacity-0')} />
                </CommandItem>
              ))}
            </CommandGroup>
          </CommandList>
