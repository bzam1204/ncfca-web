...
export function ClubDashboardTab() {
  ...
  const operational = useMemo(() => {
    ...
    const conversionRate = (approved.length + rejected.length) > 0 ? ((approved.length / (approved.length + rejected.length)) * 100).toFixed(1) + '%' : 'N/A';

    const resolvedRequests = filteredRequests.filter(r => r.resolvedAt);
    const totalResponseTime = resolvedRequests.reduce((acc, req) => acc + (new Date(req.resolvedAt!).getTime() - new Date(req.requestedAt).getTime()), 0);
  ...
  if (errorMembers || errorHistory) {
    return <Alert variant="destructive"><AlertTriangle
        className="h-4 w-4" /><AlertTitle>Erro ao Carregar Dashboard</AlertTitle><AlertDescription>{(errorMembers || errorHistory)?.message}</AlertDescription></Alert>;
  }
  ...
  return (
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <StatCard title="Tempo Médio de Resposta" value={isLoading ? "..." : operational.avgResponseTimeDays}
                    icon={Clock} />
        </div>
        ...
        {isLoading ? (
            ...
            <DashboardCharts demographics={demographics} trendData={operational.trendData} />
        )}
      </div>
  );
}