...
const EnrollmentTrendChart = ({data}: TrendChartProps) => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Volume de solicitações de matrícula aprovadas e rejeitadas ao longo do tempo.
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export function DashboardCharts({demographics, trendData}: {
  demographics: {
  ...
  const genderConfig = {
    ...
    ...Object.fromEntries(demographics.gender.map(g => [g.name, {
  ...
  return (
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <DistributionChart title="Distribuição por Gênero" description="Composição atual do clube"
                             data={demographics.gender} config={genderConfig} />
          <DistributionChart title="Distribuição por Faixa Etária" description="Composição etária do clube"
                             data={demographics.age} config={ageConfig} />
        </div>
