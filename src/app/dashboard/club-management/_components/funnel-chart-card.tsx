...
const FunnelStage = ({ icon: Icon, label, value, color, isEnd = false }: { icon: React.ElementType, label: string, value: number, color: string, isEnd?: boolean }) => (
        ...
        <CardContent className="space-y-4">
          ...
          {total > 0 && (
              <div className="text-center text-sm text-muted-foreground pt-2">
                <p><span className="font-bold">{processed}</span> de <span className="font-bold">{total}</span> solicitações foram processadas.</p>
              </div>
          )}
        </CardContent>
      </Card>
