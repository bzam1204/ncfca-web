  ...
  return (
      <Card>
        ...
        <CardContent>
          ...
          {clubQuery.isLoading && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(6)].map((_, i) => <Skeleton key={i} className="h-48 w-full" />)}
          </div>}
          {clubQuery.error && <Alert variant="destructive"><AlertTriangle
              className="h-4 w-4" /><AlertTitle>Erro ao Buscar Clubes</AlertTitle><AlertDescription>{clubQuery.error.message}</AlertDescription></Alert>}
