...
export function MyEnrollmentRequestsTable() {
  ...
  return (
    <>
            ...
            {requests.length > 0 ? (
              requests.map((req) => (
                <TableRow key={req.id} onClick={() => setSelectedRequest(req)} className="cursor-pointer">
                  ...
                  <TableCell>
                    <Badge variant={getEnrollmentStatusVariant(req.status)}>{enrollmentStatusTranslation[req.status]}</Badge>
                  </TableCell>
      ...
      <RequestDetailsDialog request={selectedRequest} onOpenChange={(isOpen) => !isOpen && setSelectedRequest(null)} />
    </>
  );
}
