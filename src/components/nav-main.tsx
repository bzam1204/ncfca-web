...
import { SidebarMenu, SidebarGroup, SidebarMenuItem, SidebarMenuButton, useSidebar } from '@/components/ui/sidebar';
    ...
    <SidebarGroup>
      <SidebarMenu>
        {props.items.map((item) => (
          <NavItem {...item} path={path} key={item.title} />
        ))}
      </SidebarMenu>
    </SidebarGroup>
  );
    ...
    <SidebarMenuItem onClick={() => isMobile && setOpenMobile(false)}>
      ...
      <SidebarMenuButton
        className={
          ...
          (isHrefCurrentPath(url, path) ? 'bg-gray-950 text-white hover:bg-gray-700 hover:text-white active:bg-gray-400 active:text-white' : '')
        }
        asChild
      >
