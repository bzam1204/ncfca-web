...
export function YouTubePlayer({ videoId, title, className, showTitle = false }: YouTubePlayerProps) {
  ...
  return (
    <div className={cn('relative w-full', className)}>
      ...
      <div
        ref={containerRef}
        className={cn('relative w-full aspect-video cursor-pointer rounded-lg overflow-hidden', 'bg-muted transition-all duration-300')}
        style={{
      ...
      >
        ...
        {!isClicked && <div className="absolute inset-0 bg-black/20 transition-opacity hover:bg-black/10" />}
