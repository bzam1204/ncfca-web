...
export class ClubGatewayApi implements ClubGateway {
    ...
    async getEnrollmentHistory(clubId: string): Promise<any[]> {
        ...
        if (!res.ok) {
            const body = await res.json();
            throw new Error(
                body.message || "Falha ao buscar histórico de matrículas"
            );
        }
        return res.json();
    }
    ...
    async getMyClubMembers(): Promise<ClubMemberDto[]> {
        ...
        if (!res.ok) {
            const body = await res.json();
            throw new Error(
                body.message || "Falha ao buscar membros do meu clube"
            );
        }
        return res.json();
    }
    ...
    async revokeMembership(membershipId: string): Promise<void> {
        const res = await fetch(
            `${this.baseUrl}/membership/${membershipId}/revoke`,
            {
                method: "POST",
                headers: { Authorization: `Bearer ${this.accessToken}` },
            }
    ...
    async approveEnrollment(enrollmentId: string): Promise<void> {
        const res = await fetch(
            `${this.baseUrl}/enrollments/${enrollmentId}/approve`,
            {
                method: "POST",
                headers: { Authorization: `Bearer ${this.accessToken}` },
            }
    ...
    async rejectEnrollment(
        ...
        const res = await fetch(
            `${this.baseUrl}/enrollments/${enrollmentId}/reject`,
            {
                method: "POST",
                headers: {
                    Authorization: `Bearer ${this.accessToken}`,
    ...
    async getPendingEnrollments(clubId: string): Promise<any[]> {
        ...
        if (!res.ok) {
            const body = await res.json();
            throw new Error(
                body.message || "Falha ao buscar matrículas pendentes"
            );
        }
        return res.json();
    }
    ...
    async getMyClubPendingEnrollments(): Promise<PendingEnrollmentDto[]> {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                body.message ||
                    "Falha ao buscar matrículas pendentes do meu clube"
            );
        }
        return res.json();
    }
